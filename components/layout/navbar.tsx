  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/70">
      <div className="container mx-auto flex min-h-16 items-center gap-4 px-4 py-3">
        <Link href="/" className="font-brand text-[1.35rem] leading-none md:text-[1.45rem]">
          MARGELLE
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-6">
          {navLinks.map((link) => (
            ...
            >
              {link.label}
              <span className="absolute inset-x-0 -bottom-2 h-px origin-left scale-x-0 bg-foreground transition-transform duration-200 group-hover:scale-x-100" />
            </Link>
          ))}
        </nav>
        ...
        <form action="/shop" className="hidden flex-1 items-center lg:flex lg:max-w-sm">
          <div className="relative w-full">
            ...
            <Input
              ...
              className="h-10 rounded-full border-border/70 bg-background pl-9 pr-4 shadow-sm transition-shadow focus-visible:border-foreground/30 focus-visible:ring-foreground/10"
            />
          </div>
        </form>
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="right" className="w-full max-w-sm">
              <div className="mt-2 space-y-6">
                <div className="flex items-center justify-between border-b pb-4 pr-10">
                  <div>
